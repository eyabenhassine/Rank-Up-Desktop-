...
//    @Override
    public void add(Reclamation reclamation) throws SQLException {
        ...
        preparedStatement.setInt(1, reclamation.getNum_Tel());
        preparedStatement.setString(2, reclamation.getNom());
        preparedStatement.setInt(3, reclamation.getNum_Tel());
    ...
    public void update(Reclamation reclamation) throws SQLException {
        // Define the SQL query with placeholders for the parameters
        String sql = "UPDATE reclamation SET nom = ?, num_Tel = ?, type = ?, description = ?, date = ? WHERE id = ?";
        ...
        preparedStatement.setInt(2, reclamation.getNum_Tel());
...
//    @Override
    public List<Reclamation> show() throws SQLException {
        ...
        String requete = "SELECT nom , type , description , date FROM reclamation";
        try (Statement st = cnx.createStatement();
            ...
            while (rs.next()) {
               ...
               // rec.setNum_Tel(rs.getInt("Num_Tel"));
    ...
    public Reclamation getOne(int id) {
        ...
        String requete = "SELECT nom, type, description , date FROM reclamation WHERE id = ?";
        try (PreparedStatement pst = cnx.prepareStatement(requete)) {
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    reclamation.setNum_Tel(rs.getInt("NumTel"));
...
//    @Override
    public ResultSet Getall() {
        ResultSet rs = null;
        try {
            String req = "SELECT nom , type , description , date FROM `reclamation`";
    ...
    public List<Reclamation> afficherRec() {
        List<Reclamation> reclamationsList = new ArrayList<>();
        String requete = "SELECT id, Nom, type, description, date FROM reclamation";
        try (Statement st = cnx.createStatement();
             ResultSet rs = st.executeQuery(requete)) {
            ...
            while (rs.next()) {
                ...
                rec.setId(rs.getInt("id"));
                rec.setNom(rs.getString("Nom").trim());
                //rec.setNum_Tel(rs.getInt("NumTel"));
                rec.setType(rs.getString("type").trim());
                rec.setDescription(rs.getString("description").trim());
                rec.setDate(rs.getDate("date").toString().trim());
    ...
    public List<Reclamation> chercher(String searchText) throws SQLException {
        ...
        String query = "SELECT * FROM reclamation WHERE Nom LIKE ? OR type LIKE ? OR description LIKE ? OR date LIKE ?";
        try (PreparedStatement st = cnx.prepareStatement(query)) {
            ...
            while (rs.next()) {
                ...
                reclamation.setId(rs.getInt("id"));
                reclamation.setNom(rs.getString("Nom"));
               // reclamation.setNum_Tel(Integer.parseInt(rs.getString("NumTel")));
                reclamation.setType(rs.getString("type"));
                reclamation.setDescription(rs.getString("description"));
                reclamation.setDate(rs.getString("date"));
    ...
    public List<Reclamation> trier(String colonne, String ordre) throws SQLException {
        ...
        String query = "SELECT Nom , Type , description , date FROM reclamation ORDER BY " + colonne + " " + ordre;
        try (Statement st = cnx.createStatement();
